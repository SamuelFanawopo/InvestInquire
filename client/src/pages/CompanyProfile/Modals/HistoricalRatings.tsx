...
interface HistoricalRatingData {
  symbol: string;
  date: string;
  rating: string;
  ratingScore: number;
...
const GET_HISTORICAL_RATING = gql`
  query HistoricalRating($ticker: String!, $page: Int!) {
    historicalRating(ticker: $ticker, page: $page) {
      symbol
      date
      rating
      ratingScore
...
const HistoricalRatings = ({
  ...
  const [loadHistoricalRating, { called, loading, data, error }] =
    useLazyQuery<{
      historicalRating: HistoricalRatingData[];
  ...
  }, [isOpen, loadHistoricalRating]);

  useEffect(() => {
    if (called) {
      loadHistoricalRating({ variables: { ticker, page } });
    }
  }, [page, loadHistoricalRating, ticker, called]);

  if (!isOpen) return null;
  if (loading) return <p>Loading...</p>;
  if (error) return <p>Error: {error.message}</p>;

  const financialData = data?.historicalRating[0];
        ...
        <div className="relative w-full max-w-2xl h-full max-h-[90vh] bg-white rounded-lg shadow dark:bg-gray-700 overflow-hidden">
          <div className="flex justify-between items-center p-5 border-b rounded-t bg-white dark:bg-gray-700 dark:border-gray-600 sticky top-0 z-10">
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
              Historical Ratings - {ticker.toUpperCase()} FY{" "}
