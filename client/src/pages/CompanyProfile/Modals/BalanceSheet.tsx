...
const GET_BALANCE_SHEET = gql`
  query BalanceSheet($ticker: String!, $page: Int!) {
    balanceSheet(ticker: $ticker, page: $page) {
      date
      symbol
      reportedCurrency
      cik
...
const BalanceSheet: React.FC<BalanceSheetProps> = ({
  ...
  const [loadBalanceSheet, { called, loading, data, error }] = useLazyQuery<{
  ...
  }, [isOpen, loadBalanceSheet]);

  useEffect(() => {
    if (called) {
      loadBalanceSheet({ variables: { ticker, page } });
    }
  }, [page, loadBalanceSheet, ticker, called]);

  if (!isOpen) return null;
  if (loading) return <p>Loading...</p>;
  if (error) return <p>Error loading data: {error.message}</p>;

  const financialData = data?.balanceSheet[0];
        ...
        <div className="relative w-full max-w-2xl h-full max-h-[90vh] bg-white rounded-lg shadow dark:bg-gray-700 overflow-hidden">
          <div className="flex justify-between items-center p-5 border-b rounded-t bg-white dark:bg-gray-700 dark:border-gray-600 sticky top-0 z-10">
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
              Balance Sheet - {ticker.toUpperCase()}{" "}
