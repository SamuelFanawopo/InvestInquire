...
const GET_CASH_FLOW_STATEMENT = gql`
  query CashFlowStatement($ticker: String!, $page: Int!) {
    cashFlowStatement(ticker: $ticker, page: $page) {
      date
      symbol
      reportedCurrency
      cik
...
const CashFlowStatement: React.FC<CashFlowStatementProps> = ({
  ...
  const [loadCashFlowStatement, { called, loading, data, error }] =
    useLazyQuery<{ cashFlowStatement: CashFlowStatementData[] }>(
  ...
  }, [isOpen, loadCashFlowStatement]);

  useEffect(() => {
    if (called) {
      loadCashFlowStatement({ variables: { ticker, page } });
    }
  }, [page, loadCashFlowStatement, ticker, called]);

  if (!isOpen) return null;
  if (loading) return <p>Loading...</p>;
  if (error) return <p>Error loading data: {error.message}</p>;

  const financialData = data?.cashFlowStatement[0];
        ...
        <div className="relative w-full max-w-2xl h-full max-h-[90vh] bg-white rounded-lg shadow dark:bg-gray-700 overflow-hidden">
          <div className="flex justify-between items-center p-5 border-b rounded-t bg-white dark:bg-gray-700 dark:border-gray-600 sticky top-0 z-10">
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
              Cash Flow Statement - {ticker.toUpperCase()}{" "}
