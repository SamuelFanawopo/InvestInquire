...
const TopLosers: React.FC = () => {
  const [openTicker, setOpenTicker] = useState<string | null>(null);
  const { loading, error, data } = useQuery<{ topLosers: TopLoser[] }>(
    GET_TOP_LOSERS,
  );
      ...
      <section className="bg-white dark:bg-gray-900 mt-12">
        <div className="py-8 px-4 mx-auto max-w-screen-xl text-center lg:pb-16">
          ...
          <p className="mt-5 text-lg font-normal text-gray-500 lg:text-xl sm:px-16 lg:px-48 dark:text-gray-400">
            Explore today's top losers, showing exchanges, names, symbols, and
            price changes. Click 'Full Data Quote' for detailed analysis.
          </p>
        </div>
      </section>
          ...
          >
            <a href="#">
              <h5 className="mb-2 text-l font-bold tracking-tight text-blue-800 dark:text-white">
                {loser.symbol}
                <span className="ml-4 text-gray-900">{loser.name}</span>
              </h5>
            </a>
            <p className="font-normal text-gray-700 dark:text-gray-400">
              USD {loser.price.toFixed(2)}
            </p>
            <p
              className={`mb-3 inline-flex font-normal ${loser.changesPercentage < 0 ? "text-red-700" : "text-green-700"} dark:text-gray-400`}
            >
              <span>
                <svg
                  className="w-6 h-6"
            ...
            <div className="mb-3">
              <button
                onClick={() => toggleIsQuoteOpen(loser.symbol)}
            ...
            {openTicker === loser.symbol && (
              <DataQuote
                key={loser.symbol}
                ticker={loser.symbol}
                isOpen={openTicker === loser.symbol}
