...
const DataQuote: React.FC<DataQuoteProps> = ({ ticker, isOpen, onClose }) => {
  const { loading, error, data } = useQuery<{ dataQuote: DataQuote[] }>(
    DATA_QUOTE_QUERY,
    {
      variables: { ticker },
    },
  ...
  const financialData = data?.dataQuote[0];
