...
const COMMODITIES_DATA_QUOTE_QUERY = gql`
  query CommoditiesDataQuote($ticker: String!) {
    commoditiesQuote(ticker: $ticker) {
      symbol
      name
      price
      changesPercentage
...
interface CommoditiesDataQuoteProps {
  ticker: string;
  isOpen: boolean;
  onClose: () => void;
}
...
const CommoditiesDataQuote: React.FC<CommoditiesDataQuoteProps> = ({
  ticker,
  isOpen,
  onClose,
}) => {
  const { loading, error, data } = useQuery<{
    commoditiesQuote: CommodityData[];
  }>(COMMODITIES_DATA_QUOTE_QUERY, {
    variables: { ticker },
  });
  ...
  const commodityData = data?.commoditiesQuote[0];
        ...
        <div className="relative w-full max-w-2xl h-full max-h-[90vh] bg-white rounded-lg shadow dark:bg-gray-700 overflow-hidden">
          ...
          >
            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                {commodityData &&
                  Object.entries(commodityData).map(([key, value]) => (
                    <tr
                      key={key}
...
export default CommoditiesDataQuote;
