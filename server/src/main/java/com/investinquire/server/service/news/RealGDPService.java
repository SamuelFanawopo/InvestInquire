...
@Service
public class RealGDPService {

    private static final Logger logger = LoggerFactory.getLogger(RealGDPService.class);
    ...
    public RealGDPService(RestTemplate restTemplate, ObjectMapper objectMapper) {
        this.restTemplate = restTemplate;
        this.objectMapper = objectMapper;
    }

    @Cacheable(value = "realGDP", key = "#page", unless = "#result == null || #result.isEmpty()")
    public List<RealGDPIndicators> getRealGDPData(Integer page) {
        int start = page * PAGE_SIZE;
        String url = "https://www.alphavantage.co/query?function=REAL_GDP&apikey=" + apiKey;
        try {
            ...
            if (responseEntity.getStatusCode().is2xxSuccessful() && responseEntity.getBody() != null) {
                ...
                if (!dataArray.isMissingNode()) {
                    return StreamSupport.stream(dataArray.spliterator(), false)
                            .map(node -> {
                                RealGDPIndicators indicator = new RealGDPIndicators();
